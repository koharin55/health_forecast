...
- 手動入力による健康データの記録
- データのグラフ表示（期間切替対応）と統計
- AIによる週次レポート生成と体調予測
- 天候データと体調の相関分析
- iOS ショートカットからのデータ自動連携
...
3. **天候情報連携**
   - OpenWeatherMap APIによる現在の天気・予報データ取得
   - Open-Meteo Archive APIによる過去データ取得（バックフィル用）
...
8. **iOS ショートカット連携**
   - REST APIエンドポイント（`POST /api/v1/health_records`）でiOSヘルスケアからデータ送信
   - APIトークン認証（Bearerトークン方式）
   - 同日データのマージ（運動・歩数・睡眠は大きい値を採用）
   - `sleep_seconds` / `exercise_seconds` パラメータ対応
   - `/guides/ios_shortcut` にセットアップガイドを掲載
...
- **外部API**:
  - OpenWeatherMap API（現在の天気・予報取得、APIキー必要）
  - Open-Meteo Archive API（過去天候データ取得、無料・APIキー不要）
  - Zipcloud API（郵便番号→住所変換）
  - Google Generative AI（Gemini API）
- **テスト**: RSpec, WebMock, FactoryBot
...
- OpenWeatherMap API キー（天候連携機能を使用する場合）
...
```bash
# Gemini API キー（AI週次レポート機能）
export GEMINI_API_KEY=your_gemini_api_key_here

# OpenWeatherMap API キー（天候連携機能）
export OPENWEATHERMAP_API_KEY=your_owm_api_key_here

# または ~/.profile に追記
echo 'export GEMINI_API_KEY=your_gemini_api_key_here' >> ~/.profile
echo 'export OPENWEATHERMAP_API_KEY=your_owm_api_key_here' >> ~/.profile
source ~/.profile
```
...
4. **データの閲覧**
   - ダッシュボード：最近の記録とグラフで推移を確認（期間切替対応）
   - 健康記録一覧：すべての記録を一覧表示
   - 詳細表示：個別の記録を詳しく確認
...
6. **CSVエクスポート / インポート**
   - 健康記録ページのエクスポートボタンからCSVダウンロード
   - インポートボタンからCSVファイルをアップロードして一括登録

7. **iOS ショートカット連携**
   - マイページでAPIトークンを発行
   - `/guides/ios_shortcut` のガイドに従い、iOSショートカットを設定
...
ユーザーの登録地域（都道府県または郵便番号）に基づき、OpenWeatherMap APIから天気を自動取得します。
...
| critical | 危険 | 76〜100 | 体調変化に十分注意してください。頭痛薬などを準備し、無理のない予定を心がけましょう。 |
...
| 気圧変化 | 現在と予報の差が大きいほどリスク上昇（最大+30）  |
| 天気 | 天気でリスク上昇  雨 → +10 / 雷雨 → +15 / 雪 → +5 |
...
| パラメータ | 型 | 説明 |
|-----------|-----|------|
| recorded_at | string | 記録日（YYYY-MM-DD。省略時は今日） |
| weight | decimal | 体重（kg） |
| sleep_seconds | integer | 睡眠時間（秒）。`sleep_minutes`に自動変換 |
| exercise_seconds | integer | 運動時間（秒）。`exercise_minutes`に自動変換 |
| steps | integer | 歩数 |
| mood | integer | 体調スコア（1-5） |

同日に複数回送信した場合、`exercise_minutes` / `steps` / `sleep_minutes` は大きい値を採用してマージします。
...
体調予測と天気取得に必要です。マイページ（`/mypage`）から以下の方法で設定できます：
...
- api_token_digest（APIトークンのハッシュ）

### HealthRecord（健康記録）
- user_id（ユーザーID）
- recorded_at（記録日）
- weight（体重）
- sleep_minutes（睡眠時間・分単位）
- exercise_minutes（運動時間・分単位）
- mood（体調スコア 1-5）
- notes（メモ）
- steps（歩数）
- heart_rate（心拍数）
...
✅ PWA化（ホーム画面追加）  
✅ プッシュ通知機能  
✅ 天候情報連携（OpenWeatherMap API + Open-Meteo Archive）  
✅ 地域設定（都道府県/郵便番号）  
✅ 過去データ天候バックフィル  
...
✅ iOS ショートカット連携 API  
✅ APIトークン管理  
✅ CSVエクスポート / インポート（健康記録）  
✅ 週次レポートのエクスポート / インポート（JSON）  
✅ ダッシュボードグラフの期間切替  

### フェーズ5（予定）
- iOSアプリ開発（Swift / HealthKit連携）
- 高リスク週の自動通知

## デプロイ

本番環境（Render）へのデプロイ時は、以下の設定が必要です：

1. 環境変数の設定
   - `RAILS_MASTER_KEY`
   - `GEMINI_API_KEY`（AI機能を使用する場合）
   - `OPENWEATHERMAP_API_KEY`（天候連携機能を使用する場合）
   - `DATABASE_URL`（PostgreSQL接続URL）

2. アセットのプリコンパイル（Dockerfileで自動実行）
